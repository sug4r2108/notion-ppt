...
class PptxBuilder:
    ...
    def _addNewSlide(self, title_text: str):
        """新しいスライドを追加し、タイトルを設定"""
    ...
    def build(self, parsed_data: list, output_file: str):
        ...
        for item in parsed_data:
            ...
            if blockType in ["heading_1", "heading_2"]:
                self._addNewSlide(item["text"])

            # 2. テキストの追加（段落や箇条書き）
            elif blockType in ["paragraph", "bulleted_list_item", "numbered_list_item"]:
                # スライドが1枚もない状態でテキストが来た場合のフェイルセーフ
                if not self.current_slide:
                    self._addNewSlide("無題のスライド")
                
                textFrame = self.body_shape.text_frame
                p = textFrame.add_paragraph()
                p.text = item["text"]
                
                # 箇条書きは少しインデントを下げる
                if blockType in ["bulleted_list_item", "numbered_list_item"]:
                    p.level = 1
                else:
                    p.level = 0
